...
## [Day Templates]
Lines: 6729–6910
Purpose: Recurring block templates, selected-day Settings skeleton editor, collapsed all-blocks advanced list, Today-row template actions, optional auto-log after scheduled end, template suppression against real/deleted entries, and hidden-on-demand Settings template form.
Functions: `generateTemplateEntries()`, `findTemplateById()`, `openTemplateLog()`, `templateSlotRange()`, `hhmmToMinutes()`, `minutesToHHMM()`, `addMinutesHHMM()`, `normalizeTemplateDays()`, `sameTemplateDays()`, `dayTemplateLabel()`, `dayTemplateFullLabel()`, `selectedDayTemplateDay()`, `templateEnergyColor()`, `templatePresetTemplates()`, `templatesForDay()`, `templateItemsForDay()`, `missingBasicsForDay()`, `renderDayTemplatePanel()`, `resetTemplateForm()`, `showTemplateForm()`, `hideTemplateForm()`, `cancelTemplateEdit()`, `startTemplateBlockForDay()`, `addBasicsToDay()`, `addBasicsToSelectedDay()`, `startTemplateBlockForSelectedDay()`, `defaultDaySkeletonTemplates()`, `templateSkeletonExists()`, `templateDuplicateKey()`, `findDuplicateTemplate()`, `addDayTemplateSkeleton()`, `addDefaultDaySkeleton()`, `templateSkippedOnDate()`, `skipTemplateOnDate()`, `autoTemplateEntryId()`, `templateSlotCovered()`, `autoLogDueTemplates()`, `entryCoversTemplateSlot()`, `scheduleTemplateDiffersFromEntry()`, `updateTemplateFromEntry()`, `maybeOfferTemplateUpdate()`, `renderTemplateList()`, `toggleTplDay()`, `persistTemplatesAfterPaint()`, `addTemplate()`, `editTemplate()`, `removeTemplate()`
