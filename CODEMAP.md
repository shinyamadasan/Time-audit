...
## [Day Templates]
Lines: 6729–6910
Purpose: Recurring block templates, flexible day-template presets, Today-row template actions, optional auto-log after scheduled end, template suppression against real/deleted entries, and Settings template editor.
Functions: `generateTemplateEntries()`, `findTemplateById()`, `openTemplateLog()`, `editTemplateFromToday()`, `templateSlotRange()`, `hhmmToMinutes()`, `minutesToHHMM()`, `addMinutesHHMM()`, `normalizeTemplateDays()`, `sameTemplateDays()`, `selectedSkeletonDays()`, `toggleSkeletonDay()`, `templatePresetDefinitions()`, `selectedTemplatePreset()`, `renderTemplatePresetHelp()`, `templatePresetTemplates()`, `defaultDaySkeletonTemplates()`, `templateSkeletonExists()`, `templateDuplicateKey()`, `findDuplicateTemplate()`, `addDayTemplateSkeleton()`, `addDefaultDaySkeleton()`, `templateSkippedOnDate()`, `skipTemplateOnDate()`, `autoTemplateEntryId()`, `templateSlotCovered()`, `autoLogDueTemplates()`, `entryCoversTemplateSlot()`, `scheduleTemplateDiffersFromEntry()`, `updateTemplateFromEntry()`, `maybeOfferTemplateUpdate()`, `renderTemplateList()`, `toggleTplDay()`, `persistTemplatesAfterPaint()`, `addTemplate()`, `editTemplate()`, `removeTemplate()`
