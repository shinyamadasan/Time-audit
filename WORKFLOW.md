...
> **âš  Migrating to the gated pipeline (DECISIONS D-015 / D-016).** Triage now *routes + enriches into
> `planning/PROPOSALS.md` (pending your approval)* â€” **never** into a build queue. The Builder builds
...
> **âš  "The Builder" below predates the Claude/Codex split (CLAUDE.md/AGENTS.md v2.0, DECISIONS D-021 /
...
> converts an approved `BUILD_QUEUE.md` item into `TASKS.md` entries (`status: codex`) â€” this can
> happen interactively (the "Plan" command) or unattended, gated behind `$AUTOMATION_ENABLED` in
> `run-claude.ps1` (see `docs/09-automation.md`) â€” and Codex then implements from `TASKS.md`. A full
...
- **Goal-driven, not priority-driven.** Every proposal is scored against the single **Current Objective**
  in `ROADMAP.md` (e.g. "alpha stability"), not raw priority â€” change that one line and the whole
...
## File map
```
root/        CLAUDE.md (router) Â· STATUS.md (working memory) Â· WORKFLOW.md Â· PROMPTS.md
             SELF_REVIEW.md (code-health gate) Â· QA.md (correctness gate)
planning/    ROADMAP.md (priority) Â· TASK.md (active task) Â· DONE.md (completed log)
captures/    inbox/ (new captures) Â· processed/YYYY/MM/ (triaged archive)
docs/        PROJECT.md Â· ARCHITECTURE.md Â· DATA_MODEL.md Â· FEATURES.md Â· DECISIONS.md
```
...
### 0. Triage (Intake)  â† runs first, every run
- **Trigger:** start of a run when `captures/inbox/` has any `*.md`. (Captures arrive from the Telegram
  bot via n8n â€” see `captures/README.md`.)
- **Who:** the agent. This is where a capture becomes an **enriched proposal** â€” it routes + enriches,
...
- **Does**, for each capture with `status: new` (SKIP any already `status: triaged` â€” idempotency):
  1. **Categorize** â€” use the `/command`; infer the type if the capture had none.
  2. **Dedupe** â€” search PROPOSALS + ROADMAP + DONE; if a dup, merge (bump the dup count), don't re-add.
  3. **Enrich into the Proposal contract** (the format in `PROPOSALS.md`), filling every field, **led
     by `â–¶ Decision`** â€” the recommended next action (Approve / Park / Reject / Clarify) + a one-line
     ...
     **evidence** (recurring friction Â· dup count Â· demand signal), **effort / dependencies / confidence
     / ambiguity**, **why now vs later**, and a **goal-adjusted** AI-recommended priority (P0..P3 â€”
...
- **Exit:** all captures triaged â†’ stop (proposals await your approval). Build only proceeds if
...
- **Does:** Implement. Keep `TASK.md` â†’ **Current Step** updated as a live progress marker â€” the resume point.
...
### 4. Self Review  â† "is this *good code*?" (precedes QA's "does it *work*?")
- **Trigger:** Execution's success criteria are met â€” but **before** Task Completion / QA / Commit.
- **Who:** the agent reviews **its own diff** as if reviewing a PR. **AI-verifiable** â€” reads the diff/codebase; no device or human judgment.
- **Does:** run `SELF_REVIEW.md` â€” the **Code Health** checklist (duplication, magic numbers, complexity, dead code, TODOs, reuse, naming, unnecessary state, unnecessary DOM queries, extract-to-helper) and answer **"Would I ship this?"**.
- **If it finds a problem:** fix it now (simplify, dedupe, delete dead code) and **re-review â€” before QA**, so QA verifies the clean version.
- **Honesty rule:** if the only hesitation on "Would I ship this?" is a **human-verified** aspect (feel/polish/animation/device), mark it `ship-pending-human-review` and log to `STATUS.md` â€” never claim it's verified.
- **Exit:** code-health clean **and** "would I ship this?" = yes â†’ **Task Completion**. ("Almost" is a no â€” fix it.)
...
- **Writes â€” the doc-sync event:**
  ...
  - `docs/DECISIONS.md` â€” new `D-0NN` if a non-obvious choice was made or reversed;
...
- **QA gate (mandatory before a production commit):** pass every **AI** check in `QA.md`
  (Functional Â· Visual Â· Regression Â· Data Integrity Â· Documentation Â· Git Hygiene). **Any AI check
  fails â†’ Blocked:** record it in `TASK.md` + `STATUS.md` and do NOT commit. Append `QA.md`'s **Human**
...
- **Who:** Mechanical FIFO â€” **promote the top of the ROADMAP Task Queue into `TASK.md`.** Priority was set by triage score + the human ordering the queue; the agent never re-chooses.
- **Writes:** the finished task is already logged in `planning/DONE.md` (at Task Completion); load the next into `TASK.md`. Queue empty â†’ `TASK.md` = `NO ACTIVE TASK`.
...
| `planning/TASK.md` | Planning (created) Â· Execution (Current Step) Â· Checkpoint (resume point) Â· Task Completion (ticked) Â· Next Task Selection (replaced) |
| `STATUS.md` | **Triage** (summary), **Checkpoint**, **Task Completion** |
| `planning/ROADMAP.md` | **Triage** (route captures) Â· Planning (add tasks) Â· Next Task Selection (promote) Â· Blocked parking |
...
Each run reads `CLAUDE.md` â†’ `STATUS.md` â†’ `planning/TASK.md`, **then runs Triage** (process
`captures/inbox/`), then:
...
| **Task blocked** | Record the blocker in `TASK.md` + `STATUS.md`. Move the task to ROADMAP **Blocked**, promote the next queue item, continue. Queue empty â†’ stop. |
...
Promoting the next FIFO item is **order-following, not prioritizing** â€” allowed. Choosing *which* task
by judgment is not. (Triage *scores* incoming captures, which sets their queue order â€” that's intake
...
Any run/session resumes by: read `STATUS.md` top entry â†’ read `planning/TASK.md` **Current Step** â†’
