...
# The app's own source files -- rendered per-app at install time (Install-AiDevOs.ps1's 'index.html', 'style.css', 'storage.js', 'focus-mode.js', 'insights.js', 'focus-wallet.js'
# token, from that app's config codeFiles key), since this is inherently different for every app.
$codeFiles = @('index.html', 'style.css', 'storage.js', 'focus-mode.js', 'insights.js', 'focus-wallet.js')
