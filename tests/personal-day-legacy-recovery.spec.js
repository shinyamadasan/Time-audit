...
async function openDevice(page, { remote = null, legacyCache = null } = {}) {
  ...
  await page.addInitScript(({ timezone, now, remote, legacyCache, path: revisionsPath }) => {
    ...
    localStorage.setItem('ta3-tz:uid_legacy-recovery-user', timezone);
    localStorage.setItem('ta3-device-id', 'device-legacy-recovery');
    localStorage.setItem('ta3-settings:uid_legacy-recovery-user', JSON.stringify({ timezone, hardMode: true, intervalMin: 30, targetRate: 250, deepGoal: 20, exitDelay: 10, presets: [], activityColors: {}, coachTone: 'analyst', reviewHour: 22, reviewTime: '22:00', sleepTime: '23:00', wakeTime: '07:00', sleepReminderMin: 30, sleepSetupDone: true, templates: [] }));
    localStorage.setItem('ta3-entries:uid_legacy-recovery-user', '[]'); localStorage.setItem('ta3-plans:uid_legacy-recovery-user', '{}'); localStorage.setItem('ta3-reviews', '{}');
