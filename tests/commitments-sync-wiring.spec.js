...
async function openSignedIn(page, { commitments = null } = {}) {
  ...
  await page.addInitScript(({ timezone, now, commitments }) => {
    ...
    localStorage.setItem('ta3-tz:uid_wiring-user', timezone);
    localStorage.setItem('ta3-device-id', 'device-wiring');
    localStorage.setItem('ta3-settings:uid_wiring-user', JSON.stringify({ timezone, hardMode: true, intervalMin: 30, targetRate: 250, deepGoal: 20, exitDelay: 10, presets: [], activityColors: {}, coachTone: 'analyst', reviewHour: 22, reviewTime: '22:00', sleepTime: '23:00', wakeTime: '07:00', sleepReminderMin: 30, sleepSetupDone: true, templates: [] }));
    localStorage.setItem('ta3-entries:uid_wiring-user', '[]'); localStorage.setItem('ta3-plans:uid_wiring-user', '{}'); localStorage.setItem('ta3-reviews', '{}');
